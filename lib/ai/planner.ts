...
import { OutfitsResponseSchema, type OutfitsResponse } from "@/lib/schemas";
...
export async function planOutfitsMock(brief: StyleBrief): Promise<OutfitsResponse> {
  const cur = (brief.currency ?? "EUR") as Currency;
  ...
  const safe: Outfit = {
    ...
    items: [
      ...
      { role: "bottom", product: prod({ id: "b1", title: "Wide-Leg Trousers", imageSeed: "safe-trousers", price: 89, currency: cur, color: "cream", availability: "in_stock", sizes: ["S","M","L"] }) },
      { role: "shoes", product: prod({ id: "s1", title: "Pointed-Toe Pumps", imageSeed: "safe-pumps", price: 110, currency: cur, color: "black", availability: "in_stock", sizes: ["37","38","39"] }) },
  ...
  const output: OutfitsResponse = { outfits: [safe, standout, wildcard] };
  // Validate before returning to guarantee JSON shape
  return OutfitsResponseSchema.parse(output);
}

export async function planOutfits(brief: StyleBrief): Promise<OutfitsResponse> {
  // Hook for OpenAI in the future. For now, always use mock (never returns empty body).
